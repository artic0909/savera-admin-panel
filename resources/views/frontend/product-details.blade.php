                    ...
                    <div class="col-lg-6 col-md-6 col-12">
                        <div class="product-single-left">
                            <div class="big-img">
                                ...
                                <img src="{{ asset('storage/' . $product->main_image) }}" alt="{{ $product->product_name }}" id="tab1" class="tabcontent" style="display: block;">
                                
                                @if(is_array($product->additional_images))
                                    @foreach($product->additional_images as $index => $image)
                                        <img src="{{ asset('storage/' . $image) }}" alt="{{ $product->product_name }}" id="tab{{ $index + 2 }}" class="tabcontent">
                                    @endforeach
                                @endif
                            </div>
                                ...
                                <div class="swiper product-tab-slider">
                                    <div class="swiper-wrapper">
                                        ...
                                        <div class="swiper-slide">
                                            <img src="{{ asset('storage/' . $product->main_image) }}" alt="{{ $product->product_name }}" class="tablinks" onclick="openCity(event, 'tab1')">
                                        </div>
                                        
                                        @if(is_array($product->additional_images))
                                            @foreach($product->additional_images as $index => $image)
                                                <div class="swiper-slide">
                                                    <img src="{{ asset('storage/' . $image) }}" alt="{{ $product->product_name }}" class="tablinks" onclick="openCity(event, 'tab{{ $index + 2 }}')">
                                                </div>
                                            @endforeach
                                        @endif
                                    </div>
                                ...
                                <div class="color-option">
                                    @if(is_array($product->colors))
                                        @foreach($product->colors as $color)
                                            ...
                                            <button style="background-color: {{ $color }};" title="{{ $color }}"></button>
                                        @endforeach
                                    @else
                                        ...
                                        if(is_array($product->metal_configurations)){
                                            foreach($product->metal_configurations as $key => $conf){
                                                 ...
                                                 if(!isset($uniqueMaterials[$mId])) {
                                                     ...
                                                     if($mObj) $uniqueMaterials[$mId] = $mObj->name;
                                                 }
                                            }
                                        }
                                     @endphp
                                     @foreach($uniqueMaterials as $mId => $mName)
                                        <button type="button" class="metal-btn" data-material-id="{{ $mId }}" onclick="selectMaterial('{{ $mId }}')">{{ $mName }}</button>
                                     @endforeach
                                </div>
                            </div>
                            <div class="ring-size">
                                ...
                                <div class="ring-size-option">
                                    <select id="size-selector" onchange="selectSize(this.value)">
                                        ...
                                        @foreach($sizes as $size)
                                            <option value="{{ $size->id }}">{{ $size->size_name ?? $size->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="product-details">
                                ...
                                <div class="div-1">
                                    @php
                                        ...
                                        $shownDiamondInfo = $defaultConfig['diamond_info'] ?? $product->diamond_gemstone_info ?? [];
                                    ...
                                    <div class="weight">
                                        ...
                                        <p>
                                            Gross Weight(Product): <span id="gross-weight">{{ $product->gross_weight }}</span> g  <br>
                                ...
                                <div class="div-1 d-block">
                                    <div id="diamond-details-wrapper" style="display: {{ (!empty($shownDiamondInfo) && is_array($shownDiamondInfo)) ? 'block' : 'none' }};">
                                        <div class="weight w-100">
                                            ...
                                            <p class="d-flex justify-content-between">
                                                <span>
                                                    <b>Total Weight <span id="diamond-total-wt">{{ $diamondTotalWt }}</span> g</b>
                                                </span>
                                                <span>
                                                    <b>Diamonds No. of <span id="diamond-total-count">{{ $totalDiamondCount }}</span></b>
                                                </span>
                                            </p>
                                ...
                                <div class="div-1 d-block">
                                    ...
                                    <div class="weight p-0 w-100" style="border: 0;">
                                        <table id="price-breakup-table">
                                            <tr>
                                                <td>
                                                    Gold <span style="font-size: 10px;" id="gold-breakdown-text">({{ number_format($netWt, 3) }}g x {{ $materialPrice }})</span>
                                                </td>
                                                <td id="gold-cost-display">
                                                    Rs. {{ number_format($materialCost, 2) }}
                                                </td>
                                            </tr>
                                            <tr id="diamond-row" style="display: {{ $hasDiamonds ? 'table-row' : 'none' }};">
                                                <td>
                                                    Diamond <span style="font-size: 10px;" id="diamond-breakdown-text">({{ number_format($diamondTotalWt, 3) }}g x {{ $diamondPricePerCarat }})</span>
                                                </td>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <b id="final-total-display">Rs. {{ number_format($finalPrice, 2) }}</b>
                                                </td>
                                            </tr>
                                        </table>
                                        <p class="mt-3">
                        ...
                        @if($similarProducts->count() > 0)
                            ...
                            <div class="col-lg-3 col-md-3 col-sm-4 col-6">
                                <div class="similar-product-item text-center">
                                    <a href="{{ route('product.show', $sProduct->id) }}" style="text-decoration: none; color: inherit;">
                                        <img src="{{ asset('storage/' . $sProduct->main_image) }}" alt="{{ $sProduct->product_name }}">
                                        <p>
...
<script>
    ...
    function updateProductDetails(config) {
        ...
        if (hasDiamonds) {
            document.getElementById('diamond-breakdown-text').textContent = `(${formatNumber(diamondTotalWt, 3)}g x ${diamondRate})`;
...
<style>
    .metal-btn.active {
        border: 2px solid #000; /* Example active style */
        font-weight: bold;
    }
</style>
@endpush